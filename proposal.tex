...
修正现有结构相似度（SSIM）指标，提出对比度加权的结构相似度（CSSIM）\cite{KINGMA2015Adam}，优化图像结构细节保留效果。
    ...
    \item 网络模型与训练策略优化\par

    采用轻量级全卷积网络作为策略网络，输入连续调图像与高斯噪声图以生成非周期半色调图案\cite{XIA2021Deep}；
设计包含 MSE、CSSIM 的复合奖励函数，兼顾色调一致性与结构细节\cite{WANG2004Image}；优化训练超参数（学习率、折扣因子等），提升框架收敛速度\cite{KINGMA2015Adam}。

    \item 模型训练与验证\par

    在 GPU 上完成模型训练，对比传统方法与现有深度学习方法的图像质量（PSNR、SSIM/CSSIM、蓝噪声特性）与计算效率\cite{ANALOUI1992Model}；
分析梯度估计器、损失函数等组件的有效性，验证框架在多色调处理等场景的可拓展性\cite{XIA2021Deep}。
...
利用策略梯度直接优化目标指标期望\cite{SUTTON2018Reinforcement}，解决梯度消失与范式不匹配问题。
    ...
    \item 策略梯度估计方差与训练稳定性问题\par

    直接应用 REINFORCE 算法会导致梯度估计方差过大。引入反事实基准与局部期望梯度估计器，
利用半色调指标的局部性特点，降低方差并提升训练稳定性\cite{FOERSTER2018Counterfactual}。

    \item 蓝噪声特性与结构细节协同优化问题\par
    
    现有方法难以同时保证蓝噪声特性（频域各向同性）与结构细节完整性。
通过频域各向异性抑制损失函数优化蓝噪声特性，结合 CSSIM 指标增强结构细节保留\cite{XIA2021Deep}。
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{graphics/总体流程图.png}
    \caption{}
\end{figure}
...
\end{equation}
    ...
    % SSIM公式
\begin{equation}
\text{SSIM}(h, c) = l(h, c) \cdot c(h, c) \cdot s(h, c)
\end{equation}
其中，$l(h,c)$、$c(h,c)$、$s(h,c)$ 分别代表亮度、对比度和结构对比系数，综合评估图像的结构相似性。
...
重点分析强化学习在离散动作输出、目标函数直接优化等方面的优势，解决半色调处理中离散像素二值化导致的梯度消失问题，为后续模型设计提供理论支撑\cite{FURUTA2020PixelRL}。
    ...
    \item {\zihao{-4}\heiti 基于深度强化学习的半色调处理框架设计}\par
    ...
    采用 Adam 优化器对模型进行训练，学习率按余弦退火方案从 3e-4 下降至 1e-5\cite{LOSHCHILOV2017SGDR}。训练过程中，Mini-batch 大小设置为 64，通过随机裁剪图像块进行训练，节省显存资源\cite{XIA2021Deep}。
